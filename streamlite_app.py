...
def send_whatsapp_message(user_name, approval_key):
    message = f"ðŸ©· HELLO XMARTY AYUSH KING SIR PLEASE â¤ï¸\nMy name is {user_name}\nPlease approve my key:\nðŸ”‘ {approval_key}"
    ...
    if pending:
        ...
        for key, info in pending.items():
            ...
            with col2:
                st.text(f"ðŸ”‘ {key}")
            with col3:
                if st.button("âœ…", key=f"approve_{key}"):
...
def approval_request_page(user_key, username):
    ...
    if st.session_state.approval_status == 'not_requested':
        ...
        with col1:
            if st.button("ðŸ“± Request Approval", use_container_width=True, key="request_approval_btn"):
                pending = load_pending_approvals()
                pending[user_key] = {
                    "name": username,
        ...
        st.code(f"""ðŸ©· HELLO XMARTY AYUSH KING SIR PLEASE â¤ï¸
My name is {username}
Please approve my key:
ðŸ”‘ {user_key}""")
        ...
        with col1:
            if st.button("ðŸ”„ Check Approval Status", use_container_width=True, key="check_approval_btn"):
                if check_approval(user_key):
                    ...
                    st.success("âœ… Approved! Redirecting...")
                    time.sleep(1)
                    st.rerun()
                else:
        ...
        with col2:
            if st.button("ðŸ”™ Back", use_container_width=True, key="back_btn"):
                st.session_state.approval_status = 'not_requested'
                st.session_state.whatsapp_opened = False
                st.rerun()
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("âœ… Login", use_container_width=True, key="admin_login_btn"):
...
def login_page():
    ...
    <div class="main-header">
        <img src="https://i.postimg.cc/Pq1HGqZK/459c85fcaa5d9f0762479bf382225ac6.jpg" class="prince-logo">
        <h1>ðŸ©·XMARTY AYUSH KING OFFLINE E2EE ðŸ¥µ</h1>
...
def main_app():
    st.markdown('<div class="main-header"><img src="https://i.postimg.cc/Pq1HGqZK/459c85fcaa5d9f0762479bf382225ac6.jpg" class="prince-logo"><h1>ðŸ©·R0W3DY E2E OFFLINEðŸ˜›</h1><p>sÉ™vÉ™n  bÄ±llÄ±on  smÄ±lÉ™s Ä±n  ÊˆhÄ±s  world  buÊˆ  É£ours Ä±s  mÉ£  fÎ±vourÄ±ÊˆÉ™s___ðŸ©·ðŸ¥µ</p></div>', unsafe_allow_html=True)
...
st.markdown('<div class="footer">Made with â¤ï¸ by Xmarty Ayush King | Â© 2025</div>', unsafe_allow_html=True)
